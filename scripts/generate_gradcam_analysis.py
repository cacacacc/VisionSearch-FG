...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Grad-CAM analysis for ResNet models.")
    ...
    parser.add_argument(
        ...
        default="predicted",
        help="Class used for Grad-CAM backpropagation.",
    )
    parser.add_argument("--alpha", type=float, default=0.45)
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/explainability/gradcam"))
...
def main() -> None:
    ...
    gradcam = ResNetGradCAM(model=model)
    output_records = []
    try:
        for index, record in enumerate(tqdm(selected_records, desc="gradcam", leave=False), start=1):
    ...
    (output_dir / "gradcam_records.json").write_text(
        json.dumps(output_records, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    write_records_csv(output_records, output_dir / "gradcam_records.csv")
...
def build_model(model_config: dict[str, Any]) -> torch.nn.Module:
    ...
    if backbone != "resnet18":
        raise ValueError("Grad-CAM analysis currently supports ResNet-18 only")
    ...
    if "projection_head" in model_config or "projection_dim" in model_config:
    ...
    for record in records:
        ...
        rows.append(
            ...
            <article class="card">
              ...
              <p>CAM target: {html.escape(record["target_class"])}</p>
              <div class="images">
                <img src="{html.escape(Path(record["original_path"]).relative_to(output_path.parent).as_posix())}" alt="original">
                <img src="{html.escape(Path(record["gradcam_path"]).relative_to(output_path.parent).as_posix())}" alt="gradcam">
              </div>
              <p class="note">人工标注：Head / Beak / Wing / Plumage / Background</p>
            </article>
            """
        )
