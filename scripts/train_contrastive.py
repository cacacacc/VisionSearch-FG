...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train CE/SupCon representation models.")
...
def main() -> None:
    ...
    pretrained = (
        ...
        else model_config.get("pretrained", True)
    )
    max_train_batches = (
    ...
    if ce_weight == 0 and supcon_weight == 0:
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_stats = train_one_epoch_contrastive(
            model=model,
...
def train_one_epoch_contrastive(
    ...
    for batch in progress:
        ...
        if supcon_weight > 0:
            ...
            supcon_loss = zero_like_loss(output.logits)
...
def zero_like_loss(tensor: torch.Tensor) -> torch.Tensor:
...
def build_optimizer_parameter_groups(
    ...
    projection_parameters = [p for p in model.projection_head.parameters() if p.requires_grad]
...
def _limit_batches(dataloader: DataLoader, max_batches: int | None):
